...
_Last updated: 2026-07-21. This document is completed by a human before tagging
a release; do not treat “Met” as a substitute for the final review._
...
| 4 | Fresh install starts with no collector/album data and exactly one restricted first-admin | Met | #27 bootstrap; CI deployment asserts empty catalog/profiles/holdings |
| 5 | WM2026 catalog and example holdings load only via explicit seeds | Met | #17 (`seed:wm2026`), #18 (example holdings); never auto-run |
...
| 15 | Core workflows usable at iPhone 13 size | **Pending** | Automated mobile acceptance at iPhone 13 — tracked by **#44** |
| 16 | Load test with ~50 concurrent users passes without domain errors | **Pending** | 50-user load test + p95 targets — tracked by **#43** |
...
- **Fresh installation** on the target: `./start.sh` (or `docker compose up -d --build`) on an empty database creates exactly one restricted admin; confirm the forced first-admin password change before any admin function is reachable.
...
- **Seeds**: `pnpm seed:wm2026` is idempotent; the example-holdings seed requires an explicit collector and never runs automatically.
